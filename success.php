<?php $title = 'Success';
     require_once 'includes/header.php'; 
     require_once 'db/conn.php';
     require_once 'sendemail.php';

    if (isset($_POST['submit'])){
        //Extract Values from the $_POST Array
        $fname = $_POST['firstname'];
        $lname = $_POST['lastname'];
        $sex = $_POST['sex'];
        $yard = $_POST['yard'];
        $dob = $_POST['dob'];
        $email = $_POST['email'];
        $contact = $_POST['phone'];
        $tof = $_POST['tof'];

        $orig_file = $_FILES["avatar"]["tmp_name"];
        $ext = pathinfo($_FILES["avatar"]["name"], PATHINFO_EXTENSION);
        $target_dir = 'uploads/';
        $destination = "$target_dir$contact.$ext";
        move_uploaded_file($orig_file,$destination);

         //Call function to insert and track if success or not
       $isSuccess = $crud->insertFans($fname, $lname, $sex, $yard, $dob, $email, $contact, $tof,$destination);
       $tofName = $crud->getTofById($tof);

       if($isSuccess){
          // SendEmail::SendMail($email,'Welcome Fans  ','You have successfully registered for Manchester United Fan Club');
        include 'includes/successmessage.php';

       }
       else{
        include 'includes/errormessage.php';

       }
    }
    ?>
    
<!-- This Prints out values passed to the action page using method="post" -->
<img src="<?php echo $destination; ?>" class="dashboard-profile-img rounded-circle" style="width: 45%; height: 45%" />
<div class="card" style="width: 18rem;">
        <div class="card-body">
            <h5 class="card-title"><?php echo $_POST['firstname'].' '.$_POST['lastname']; ?></h5>
            <h6 class="card-subtitle mb-2 text-muted"><?php echo $tof['tof']; ?></h6>
            <p class="card-text">
                Sex: <?php echo $_POST['sex']; ?></p>
                <p class="card-text">
                Address: <?php echo $_POST['yard']; ?></p>
            <p class="card-text">
                Date Of Birth: <?php echo $_POST['dob']; ?></p>
            <p class="card-text">
                Email Address: <?php echo $_POST['email']; ?></p>
                <p class="card-text">
                Contact Number: <?php echo $_POST['phone']; ?></p>
            
        </div>
    </div>



<br>
<br>
<br>
<?php require_once 'includes/footer.php'; ?>